...
export function BeanFormPage({ beanId }: { beanId?: string }) {
  const [name,setName]=useState(""); const [roaster,setRoaster]=useState(""); const [origin,setOrigin]=useState("");
  ...
  const [isGround,setIsGround]=useState(false); const [photo,setPhoto]=useState<Blob>(); const [taste,setTaste]=useState<TasteValues>({});
  ...
  useEffect(() => {
    ...
    void getBean(beanId).then((b) => {
      if (!b) return;
      setName(b.name); setRoaster(b.roaster ?? ""); setOrigin(b.origin ?? ""); setRoastLevel(b.roastLevel ?? "");
      setProcess(b.process ?? ""); setVariety(b.variety ?? ""); setPurchaseDate(b.purchaseDate ?? "");
      setPrice(b.price?.toString() ?? ""); setWeight(b.weight?.toString() ?? ""); setIsGround(b.isGround ?? false);
      setPhoto(b.photo); setTaste({ acidity:b.acidity, bitterness:b.bitterness, sweetness:b.sweetness, body:b.body, aroma:b.aroma });
  ...
  async function save() {
    ...
    const bean: Bean = {
      id: beanId ?? crypto.randomUUID(), name:name.trim(), roaster:roaster.trim()||undefined, origin:origin.trim()||undefined,
      roastLevel:roastLevel||undefined, process:process.trim()||undefined, variety:variety.trim()||undefined,
      purchaseDate:purchaseDate||undefined, price:price?Number(price):undefined, weight:weight?Number(weight):undefined,
      isGround, photo, ...taste, overallRating:overall, repeatStatus:repeat, favorite, memo:memo.trim()||undefined,
      createdAt, updatedAt:now
    };
    try { await saveBean(bean); navigate(`/beans/${bean.id}`); } catch { setError("保存に失敗しました。"); setSaving(false); }
  }

  return <div className="space-y-5">
    <KuraCard><div className="space-y-5"><PhotoInput value={photo} onChange={setPhoto}/>
      ...
      <KuraInput label="産地" value={origin} onChange={setOrigin} placeholder="例：エチオピア イルガチェフェ"/>
      ...
      <KuraInput label="購入日" value={purchaseDate} onChange={setPurchaseDate} type="date"/>
